...
context("Grammar with a missing rule")
...
Parser <- R6Class("Parser",
  public = list(
    ...
    p_statement_assign = function(doc='statement : location EQUALS expression', t) {
      names[t[1]] <- t[3]
    },
...
test_that("rule", {
    expect_error(rly::yacc(Parser), "ERROR> Symbol location used, but not defined as a token or a rule", fixed=TRUE)
  })
